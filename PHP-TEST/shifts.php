...
$stmt = $mysqli->prepare('SELECT `s`.`shift_id` AS `id`, `s`.`user_id` AS `userId`, `u`.`full_name` AS `cashierName`, `s`.`time_in` AS `loginTimestamp`, `s`.`time_out` AS `logoutTimestamp`, `s`.`shift_duration_seconds` AS `shiftDurationSeconds`, `s`.`total_sales` AS `totalSales` FROM `cashier_shift` `s` JOIN `user` `u` ON `u`.`user_id` = `s`.`user_id` ORDER BY `s`.`time_in` DESC LIMIT ? OFFSET ?');
